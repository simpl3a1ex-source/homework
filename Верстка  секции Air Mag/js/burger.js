export default class BurgerMenu {
	constructor(config, headerFixedInstance = null) {
		this.config = config;
		this.burgerButton = document.querySelector(
			`.${this.config.BURGER}`,
		);
		this.burgerMenu = document.querySelector(
			`.${this.config.HEADER_MENU}`,
		);
		this.body = document.querySelector(
			`.${this.config.PAGE_BODY}`,
		);
	...
	removeEvents() {
		// Click events
		this.burgerButton.removeEventListener(
			"click",
			this.onBurgerClick,
		);
	...
	onBurgerClick() {
		...
		this.burgerMenu.classList.toggle(
			this.config.HEADER_MENU_OPEN,
			isOpen,
		);
		this.body.classList.toggle(
			this.config.PAGE_BODY_NO_SCROLL,
			isOpen,
		);
	...
	hideBurgerMenu() {
		...
		this.burgerButton.classList.remove(
			this.config.BURGER_OPEN,
		);
	...
	isBurgerMenuOpen() {
		return this.burgerMenu.classList.contains(
			this.config.HEADER_MENU_OPEN,
		);
	}
	...
	onBodyClick(event) {
		...
		const isLinkInMenu = target.classList.contains(
