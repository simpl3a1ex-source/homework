export default class Modal {
	...
	close() {
		...
		this.modalWindow.classList.remove(
			"modal__window--open",
		);
	...
	catchFocus(event) {
		const focusableElements =
			this.modalWindow.querySelectorAll(this.focusElements);
		const focusArray = Array.prototype.slice.call(
			focusableElements,
		);
		const focusIndex = focusArray.indexOf(
		...
		if (
			!event.shiftKey &&
			focusIndex === focusArray.length - 1
		) {
			focusArray[0].focus();
			event.preventDefault();
		}
